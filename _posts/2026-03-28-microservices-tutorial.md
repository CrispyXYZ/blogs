...
在单体架构中，由于所有模块紧密耦合，技术选型往往被“锁死”。
- 痛点描述：当团队想要针对特定模块（如订单模块）引入新技术或进行版本升级时。
- 后果：由于代码库是一个整体，无法实现局部升级，往往必须重写整个项目才能完成技术迭代。 ❌

**场景 2：单点故障**

由于所有功能运行在同一个进程中，局部的错误会导致灾难性的连锁反应。
- 痛点描述：订单模块出现 Bug（如内存溢出或死循环）。
- 后果：故障会迅速蔓延至全局，导致整个系统崩溃。即使是无关的模块（如用户登录）也会随之失效，用户无法正常使用。 ❌

**场景 3：性能瓶颈**

单体架构在应对高并发压力时，扩展效率极低。
- 过程描述：
  - 用户反馈：大量用户抱怨“网站好卡！”。
  - 扩展尝试：运维团队尝试扩展服务器硬件。
  - 发现障碍：由于用户模块和订单模块的代码物理上混在一起，无法只针对压力大的模块进行扩容。
- 后果：资源无法精准分配，系统难以有效扩展。 ❌

总结：这三个场景共同指向了微服务架构转型的必要性，即通过解耦来提升系统的可维护性、容错性和伸缩性。
...
**六大核心模块**: 这些模块在项目根目录的 `docker-compose.yml` 中定义，使用 Docker Compose 可以一键启动所有服务。如果你不熟悉 Docker，不用担心，[附录 C](#c-常用命令) 提供了常用命令和容器基本概念，帮助你快速上手。
